...
def count_fingers_single_hand(hand_landmarks):
    ...
    return max(MIN_ROBOT_FINGERS, min(MAX_ROBOT_FINGERS, sum(fingers)))
...
def start_new_exercise():
    global first_number, second_number, correct_answer, math_state, state_start_time, candidate_answer, candidate_start_time
    ...
    math_state, state_start_time = "SHOW_FIRST", time.time()
    ...
    with mp_hands.Hands(model_complexity=1, max_num_hands=2, min_detection_confidence=0.8, min_tracking_confidence=0.8) as hands:
        while cap.isOpened():
            ...
            if math_state == "CHOOSE_OPERATION":
                cv2.putText(frame, "Math Mode", (30, 80), cv2.FONT_HERSHEY_SIMPLEX, 1.4, (255, 255, 255), 3)
                cv2.putText(frame, "Show 1 Finger = Addition", (30, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 255), 2)
                cv2.putText(frame, "Show 2 Fingers = Subtraction", (30, 240), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 255), 2)
                cv2.putText(frame, "👌 (OK Sign): Return to Education Menu", (30, 310), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (200, 200, 200), 2)
                ...
                if elapsed >= ROBOT_SHOW_SECONDS:
                    send_robot_fingers(0)
                    math_state, state_start_time = "GAP", current_time

            elif math_state == "GAP":
                if elapsed >= ROBOT_GAP_SECONDS:
                    send_robot_fingers(second_number)
                    math_state, state_start_time = "SHOW_SECOND", current_time

            elif math_state == "SHOW_SECOND":
                if elapsed >= ROBOT_SHOW_SECONDS:
                    send_robot_fingers(0)
                    candidate_answer, candidate_start_time, math_state, state_start_time = None, 0, "WAIT_FOR_ANSWER", current_time
                ...
                if current_finger_count != -1:
                    if candidate_answer != current_finger_count:
                        ...
                        math_state, state_start_time, last_state_change_time = "FEEDBACK", current_time, current_time
                ...
                if elapsed >= FEEDBACK_SECONDS:
                    math_state, state_start_time, last_state_change_time = "ROUND_END_MENU", current_time, current_time

            elif math_state == "ROUND_END_MENU":
                cv2.putText(frame, "👍 (Thumbs Up) to continue/retry", (30, 230), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 255), 2)
                cv2.putText(frame, "👌 (OK Sign): return to Education Menu", (30, 285), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 255), 2)
                ...
                if current_time - last_state_change_time > BACK_COOLDOWN_SECONDS:
                    if current_ok_gesture:
                        ...
                        else: candidate_answer, candidate_start_time, math_state, state_start_time, last_state_change_time = None, 0, "WAIT_FOR_ANSWER", current_time, current_time
