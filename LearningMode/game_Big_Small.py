...
try:
    with mp_hands.Hands(model_complexity=1, max_num_hands=2, min_detection_confidence=0.8, min_tracking_confidence=0.8) as hands:
        while cap.isOpened():
            ...
            if state == "CHOOSE_MODE":
                draw_lines(frame, ["GREATER / SMALLER MODE"], 65, TITLE_COLOR, scale=1.1, thickness=3)
                draw_lines(frame, ["Show 1 finger = Greater", "Show 2 fingers = Smaller"], 145, OPTION_COLOR, scale=0.95, thickness=2)
                draw_lines(frame, ["Voice: say GREATER / SMALLER", "OK sign = Back to Education Menu", "Voice: BACK"], 260, NOTE_COLOR, scale=0.76, thickness=2, step=40)
                desired_action = None
                if current_ok:
                    desired_action = "BACK"
                ...
                draw_lines(frame, ["GREATER / SMALLER MODE"], 65, TITLE_COLOR, scale=1.1, thickness=3)
                draw_lines(frame, [f"Chosen mode: {selected_mode}", "Thumbs up = Start game", "Voice: START"], 145, OPTION_COLOR, scale=0.88, thickness=2)
                draw_lines(frame, ["OK sign = Back to mode choice", "Voice: BACK"], 275, NOTE_COLOR, scale=0.76, thickness=2, step=40)
                ...
                draw_lines(frame, [f"Mode: {selected_mode}", "OK sign = Back to mode choice", "Voice: BACK"], 240, NOTE_COLOR, scale=0.76, thickness=2, step=40)
                ...
                draw_lines(frame, [f"Question: Which is {selected_mode}?", "OK sign = Back to mode choice", "Voice: BACK"], 240, NOTE_COLOR, scale=0.76, thickness=2, step=40)
                desired_action = "BACK" if current_ok else None
                action, _ = update_hold(desired_action, current_time)
                if action == "BACK":
                    go_to_mode_menu()
                ...
                draw_lines(frame, ["ANSWER THE QUESTION"], 65, TITLE_COLOR, scale=1.1, thickness=3)
                draw_lines(frame, [f"Numbers: {first_number} and {second_number}"], 140, OPTION_COLOR, scale=0.95, thickness=2)
                draw_lines(frame, [f"Show the {selected_mode.lower()} number", "OK sign = Back to mode choice", "Voice: BACK"], 215, NOTE_COLOR, scale=0.76, thickness=2, step=40)
                ...
                draw_lines(frame, [msg, "Voice: START", "OK sign = Back to mode choice", "Voice: BACK"], 220, OPTION_COLOR, scale=0.78, thickness=2, step=40)
                desired_action = "START" if current_thumbs else ("BACK" if current_ok else None)
                action, _ = update_hold(desired_action, current_time)
                if action == "START":
                    if last_answer_correct:
