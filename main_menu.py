...
def draw_lines(frame, lines, start_y, color, scale=0.9, thickness=2, step=50):
    for i, line in enumerate(lines):
        cv2.putText(frame, line, (35, start_y + i * step), cv2.FONT_HERSHEY_SIMPLEX, scale, color, thickness)
...
def draw_hold_status(frame, current_time):
    ...
    if hold_action:
        ...
        cv2.putText(frame, f"Hold action: {hold_label(hold_action)}", (35, h - 95), cv2.FONT_HERSHEY_SIMPLEX, 0.75, HOLD_COLOR, 2)
        cv2.putText(frame, f"Hold selection: {progress:.1f}s / {SELECTION_HOLD_SECONDS:.1f}s", (35, h - 55), cv2.FONT_HERSHEY_SIMPLEX, 0.75, HOLD_COLOR, 2)
    else:
        cv2.putText(frame, "Hold a gesture for 1.5 seconds to select", (35, h - 55), cv2.FONT_HERSHEY_SIMPLEX, 0.75, NOTE_COLOR, 2)


def draw_status(frame, current_fingers, voice_text):
    h, _, _ = frame.shape
    if current_fingers != -1:
        cv2.putText(frame, f"Detected fingers: {current_fingers}", (35, h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, STATUS_COLOR, 2)
    if voice_text:
        cv2.putText(frame, f"Voice: {voice_text}", (35, h - 130), cv2.FONT_HERSHEY_SIMPLEX, 0.68, STATUS_COLOR, 2)


def draw_main_menu(frame):
    draw_lines(frame, ["MAIN MENU"], 70, TITLE_COLOR, scale=1.2, thickness=3)
    draw_lines(frame, ["Show 1 finger = Game Mode", "Show 2 fingers = Learning Mode"], 145, OPTION_COLOR, scale=0.95, thickness=2, step=55)
    draw_lines(frame, ["Voice: say GAME or LEARNING", "OK sign = Back (in sub menus)"], 280, NOTE_COLOR, scale=0.75, thickness=2, step=40)


def draw_game_menu(frame):
    draw_lines(frame, ["GAME MODE"], 70, TITLE_COLOR, scale=1.2, thickness=3)
    draw_lines(frame, ["Show 1 finger = Rock Paper Scissors", "Show 2 fingers = Even Odd"], 145, OPTION_COLOR, scale=0.9, thickness=2, step=55)
    draw_lines(frame, ["OK sign = Back to Main Menu", "Voice: ROCK / EVEN / BACK"], 280, NOTE_COLOR, scale=0.75, thickness=2, step=40)


def draw_learning_menu(frame):
    draw_lines(frame, ["LEARNING MODE"], 70, TITLE_COLOR, scale=1.2, thickness=3)
    draw_lines(frame, ["Show 1 finger = Counting / Imitation", "Show 2 fingers = Addition / Subtraction", "Show 3 fingers = Greater / Smaller"], 140, OPTION_COLOR, scale=0.85, thickness=2, step=50)
    draw_lines(frame, ["OK sign = Back to Main Menu", "Voice: COUNTING / MATH / GREATER / BACK"], 310, NOTE_COLOR, scale=0.75, thickness=2, step=40)
...
def handle_voice_command(text):
    ...
    if menu_state == MAIN_MENU:
        ...
        if "COUNT" in text or "IMITATION" in text:
            ...
            return True
        if "MATH" in text or "PLUS" in text or "MINUS" in text or "ADD" in text or "SUB" in text:
...
try:
    with mp_hands.Hands(model_complexity=1, max_num_hands=1, min_detection_confidence=0.8, min_tracking_confidence=0.8) as hands:
        while True:
            ...
            draw_status(frame, current_fingers, latest_voice if voice_enabled else "voice unavailable")
